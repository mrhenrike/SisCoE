...
import GUI_Dialogs_Devolucao.Conf_Sair_Sem_Salvar_Dev_Outra;
import GUI_Dialogs_Devolucao.Conf_Salvar_Dev_Outra;
import GUI_Dialogs_Devolucao.Inf_Dados_Nao_Salvos_Dev_Outra;
import GUI_Dialogs_Devolucao.Inf_Dados_Salvos_Dev_Outra;
import GUI_Dialogs_Devolucao.Inf_Dev_Ja_Efetivada_Dev_Outra;
import GUI_Dialogs_Devolucao.Inf_Nao_Existe_Prod_Dev_Outra;
import GUI_Dialogs_Devolucao.Inf_Preencher_Campos_Dev_Outra;
import GUI_Dialogs_Devolucao.Inf_Saida_Nao_Encontrada_Dev_Outra;
...
public class Tela_Gerar_Devolucao_Outra extends javax.swing.JInternalFrame {

    //metodo para abrir apenas uma instancia da janela
    public static Tela_Gerar_Devolucao_Outra Obj;
    
    public void Open_Tela(){
        if(Obj==null){
            Obj = new Tela_Gerar_Devolucao_Outra();
    ...
    private static Inf_Dados_Salvos_Dev_Outra ObjDadosSalvos;
    private static Inf_Dados_Nao_Salvos_Dev_Outra ObjDadosNaoSalvos;
    private static Inf_Saida_Nao_Encontrada_Dev_Outra ObjSaidaNaoEncontrada;
    private static Inf_Preencher_Campos_Dev_Outra ObjPreencherCampos;
    private static Inf_Nao_Existe_Prod_Dev_Outra ObjNaoExisteProdDev;
    private static Inf_Dev_Ja_Efetivada_Dev_Outra ObjDevJaEfetivada;
    private static Conf_Salvar_Dev_Outra ObjSalvar;
    private static Conf_Sair_Sem_Salvar_Dev_Outra ObjSairSemSalvar;
    private static Tela_Consulta_Saida_Devolucao_Outra_DL ObjConsSaida;
    ...
    public Tela_Gerar_Devolucao_Outra() {
        initComponents();
        Desabilita_Campos();
        ...
        JTB_Devolucao_Prod.setEnabled(false);
        Setar_Atalho_BT();
    }
    ...
    private void initComponents() {
        ...
        setIconifiable(true);
        setTitle("Devolução De Produto - Outras Saídas");
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(JTF_Observacao, javax.swing.GroupLayout.DEFAULT_SIZE, 688, Short.MAX_VALUE)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addComponent(JTF_Tipo, javax.swing.GroupLayout.PREFERRED_SIZE, 241, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    public void Testar_Campos_Pesquisa(){
        if(!JTF_Pesquisa.getText().equalsIgnoreCase("")){//se o campo nao estiver em branco
            ObjControlSaidaOutra.Consulta_Saida_Id(Integer.parseInt(JTF_Pesquisa.getText()));//consulta se existe pelo menos uma saída
            if(ObjControlSaidaOutra.Controle_Saida == true){//se existir saída
                  ObjControlSaidaOutra.Verifica_Saida_Devolucao(JTF_Pesquisa);//verifica se precisa realizar devolução
                  if(ObjControlSaidaOutra.Verifica_Devolucao == true){//se precisar devolver 
                    ObjControlSaidaOutra.Saida_Em_Aberto(Integer.parseInt(JTF_Pesquisa.getText()));//verifica se esta em aberto
                        if(ObjControlSaidaOutra.Controle_Saida_Em_Aberto == true){//se estiver em aberto, so preenche com os produto que ainda nao foram devolvidos
                            ...
                            + " inner join saida_itens_outra on produto.id_produto = saida_itens_outra.produto_id_produto "
                            + " where saida_outra_id_saida_outra = "+JTF_Pesquisa.getText()+" and solicita_devolucao = 'SIM' and devolvido = 'NÃO'");
                            Limpar_Campos();
                            ObjControlSaidaOutra.Consulta_Saida_Devolucao(ObjModeloSaida, Integer.parseInt(JTF_Pesquisa.getText()));
                            Setar_Campos();
                            BT_Salvar.setEnabled(true);
                            ObjControlSaidaOutra.Verifica_Devolucao= false;
                            JTF_Pesquisa.setText("");
                            ObjControlSaidaOutra.Controle_Saida_Em_Aberto = false;
                        }else{//se naão estiver em aberto, preenche normal                     
                            Limpar_Tabela(JTB_Devolucao_Prod);
                            Preencher_Tabela("select * from produto inner join categoria_produto "
                            + " on produto.Categoria_Produto_id_categoria = categoria_produto.id_categoria "
                            + " inner join saida_itens_outra on produto.id_produto = saida_itens_outra.produto_id_produto "
                            + " where saida_outra_id_saida_outra = "+JTF_Pesquisa.getText()+" and solicita_devolucao = 'SIM' ");
                            Limpar_Campos();
                            ObjControlSaidaOutra.Consulta_Saida_Devolucao(ObjModeloSaida, Integer.parseInt(JTF_Pesquisa.getText()));
                            Setar_Campos();
                            BT_Salvar.setEnabled(!true);
    ...
    public void Testar_Campos_Salvar(){
        ObjControlSaidaOutra.Verifica_Devolucao_Efetivada(JL_Saida_Numero.getText());
        if(ObjControlSaidaOutra.Confirma_Devolucao==true){
            Mostrar_Dev_Ja_Efetivada();
            ObjControlSaidaOutra.Confirma_Devolucao=false;
        }else{
    ...
    public void Conf_Inserir_Devolucao(){
        ...
        if(Confirma_Devolucao == true){
            ...
            new Controle_Log().Registrar_Log("efetivou a devolução referente a saída (outra) id: "+JL_Saida_Numero.getText()
                    +" - "+ObjModeloSaida.getTipo(), CodLogado);
            int id = Integer.parseInt(JL_Saida_Numero.getText());
            ObjControlSaidaOutra.Consulta_Saida_Devolucao(ObjModeloSaida, id);
            Limpar_Campos();
            Setar_Campos();
            BT_Salvar.setEnabled(false);
            ObjControlSaidaOutra.Verifica_Devolucao= false;
            ...
            new Controle_Log().Registrar_Log("erro ao efetivar a devolução referente a saída (outra) id: "+JL_Saida_Numero.getText()
                    +" - "+ObjModeloSaida.getTipo(), CodLogado);
        }
    ...
    try {
        ...
        //Laço para fazer todas as inserçoes no banco de entrada de itens
            for(int Linha = 0; Linha < Quant_Linhas; Linha++){
                try {
                    JTB_Devolucao_Prod.setRowSelectionInterval(Linha,Linha); //seta na primeira linha da tabela
                    ...
                    if(JTB_Devolucao_Prod.getValueAt(Linha, 4).equals("")){   //Produto sem validade 
                        ...
                        ObjControlSaidaOutra.Atualiza_Produto_Devolvido(Id_Produto, "SIM");//atualiza o produto para devolvido
                    }
                    else{//Com validade
                        int Id_Produto = (Integer.valueOf(String.valueOf(JTB_Devolucao_Prod.getValueAt(Linha, 0))));//Pega o id do produto na linha da tabela
                        double Quant = Double.parseDouble(String.valueOf(JTB_Devolucao_Prod.getValueAt(Linha, 3)));//pega a quantidade na linha da tabela
                        ...
                        ObjControlSaidaOutra.Atualiza_Produto_Devolvido_Lote(Id_Produto, Lote, "SIM");
                    }
            ...
            ObjControlSaidaOutra.Efetivar_Situacao(JL_Saida_Numero.getText(),"EFETIVADA DEVOLUÇÃO");
            ObjControlSaidaOutra.Atualiza_Data_Alteracao_Saida(Integer.parseInt(JL_Saida_Numero.getText()));//altera a data de alteração
    ...
    public void Mostrar_Dados_Salvos(){
        ObjDadosSalvos = new Inf_Dados_Salvos_Dev_Outra(this, true);
        ObjDadosSalvos.setVisible(true);
    }
    public void Mostrar_Dados_Nao_Salvos(){
        ObjDadosNaoSalvos = new Inf_Dados_Nao_Salvos_Dev_Outra(this, true);
    ...
    public void Mostrar_Preencher_Campos(){
        ObjPreencherCampos = new Inf_Preencher_Campos_Dev_Outra(this, true);
        ObjPreencherCampos.setVisible(true);
    }
    public void Mostrar_Nao_Existe_Prod_Dev(){
        ObjNaoExisteProdDev = new Inf_Nao_Existe_Prod_Dev_Outra(this, true);
        ObjNaoExisteProdDev.setVisible(true);
    }
    public void Mostrar_Dev_Ja_Efetivada(){
        ObjDevJaEfetivada = new Inf_Dev_Ja_Efetivada_Dev_Outra(this, true);
        ObjDevJaEfetivada.setVisible(true);
    }
    public void Mostrar_Confirma_Salvar(){
        ObjSalvar = new Conf_Salvar_Dev_Outra(this, true);
        ObjSalvar.setVisible(true);
    }
    public void Mostrar_Sair_Sem_Salvar(){
        ObjSairSemSalvar = new Conf_Sair_Sem_Salvar_Dev_Outra(this, true);
        ObjSairSemSalvar.setVisible(true);
    }
    public void Mostrar_Consulta_Saida(){
        ObjConsSaida = new Tela_Consulta_Saida_Devolucao_Outra_DL(this, true);
