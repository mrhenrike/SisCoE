...
import GUI_Dialogs_Relatorios.Inf_Data_Final_Inferior_Relat_Mov_Prod_Saida_Turma;
import GUI_Dialogs_Relatorios.Inf_Preencher_Datas_Relat_Mov_Prod_Saida_Turma;
...
public class Tela_Relat_Mov_Prod_Saida_Turma extends javax.swing.JInternalFrame {
    
     public static Tela_Relat_Mov_Prod_Saida_Turma Obj;
    
    public void Open_Tela(){
        if(Obj==null){
            Obj = new Tela_Relat_Mov_Prod_Saida_Turma();
    ...
    private static Inf_Preencher_Datas_Relat_Mov_Prod_Saida_Turma ObjPreencherDatas;
    private static Inf_Data_Final_Inferior_Relat_Mov_Prod_Saida_Turma ObjDataInferior;
    
    String sql_semestre = " ";
    String ano;
    
    public Tela_Relat_Mov_Prod_Saida_Turma() {
        initComponents();
    ...
    private void initComponents() {
        ...
        setIconifiable(true);
        setTitle("62 - Movimentação De Produto Por Saída Por Turma");
        ...
        if(JD_Inicial.getDate()==null || JD_Final.getDate()==null){
            ...
            if(DataMenor == true){
                ...
                if (JCB_Curso.getSelectedIndex() == 0) {//todos cursos
                    ano = String.valueOf(JY_Ano.getValue());
                    new Controle_Relatorio_Produto().Relatorio_Produto_Mov_Saida_Periodo_Turma(JD_Inicial, JD_Final, sql_semestre, ano);
                    ...
                    new Controle_Log().Registrar_Log("Gerou o relatório de movimentacao de produto por saída por turma todas de " + ano + " no período de " + dti + " até " + dtf, CodLogado);
                } else {//curso selecionado
                    ano = String.valueOf(JY_Ano.getValue());
                    String curso = JCB_Curso.getSelectedItem().toString().trim();
                    if (JCB_Turma.getSelectedIndex() == 0) {//todas turmas                        
                        new Controle_Relatorio_Produto().Relatorio_Produto_Mov_Saida_Periodo_Turma_Curso_Selecionado(JD_Inicial, JD_Final, sql_semestre, ano,curso);
                        String dti = new SimpleDateFormat("dd-MM-yyyy").format(JD_Inicial.getDate());
                        String dtf = new SimpleDateFormat("dd-MM-yyyy").format(JD_Final.getDate());
                        new Controle_Log().Registrar_Log("Gerou o relatório de movimentacao de produto por saída por turma todas de " + ano + " - curso: '" + curso + "' no período de " + dti + " até " + dtf, CodLogado);
                    } else {//turma selecionada
                        Id_Turma();//pega o id da turma
                        new Controle_Relatorio_Produto().Relatorio_Produto_Mov_Saida_Periodo_Turma_Selecionada_Curso_Selecionado(JD_Inicial, JD_Final, sql_semestre, ano, curso, ObjModTurma.getId_turma());
                        String dti = new SimpleDateFormat("dd-MM-yyyy").format(JD_Inicial.getDate());
                        String dtf = new SimpleDateFormat("dd-MM-yyyy").format(JD_Final.getDate());
                        new Controle_Log().Registrar_Log("Gerou o relatório de movimentacao de produto por saída por turma selecionada id: " + ObjModTurma.getId_turma() + " - '" 
                                + JCB_Turma.getSelectedItem().toString() + "' de " + ano + " - curso: '" + curso + "' no período de " + dti + " até " + dtf, CodLogado);
                    }
                }
    ...
    public void Mostrar_Preencher_Datas(){
        ObjPreencherDatas = new Inf_Preencher_Datas_Relat_Mov_Prod_Saida_Turma(this, true);
        ObjPreencherDatas.setVisible(true);
    }
    public void Mostrar_Data_Inferior(){
        ObjDataInferior = new Inf_Data_Final_Inferior_Relat_Mov_Prod_Saida_Turma(this, true);
